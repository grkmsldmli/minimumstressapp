...
/**
 * Growth: the audited live funnel, plus a truthful summary of the product-event
 * stream over the last 30 days. Surfaces with no collector are named as
 * uninstrumented, never charted as zero.
 */
export function GET(): Promise<Response> {
  return adminGet(async (admin) => {
    const since = new Date(Date.now() - WINDOW_DAYS * 24 * 60 * 60 * 1000).toISOString();
    const [q, eventsRes] = await Promise.all([
      loadReportingQueue(admin),
      admin
